...
def _extend_dataframe(initial_dataframe, new_rows):
    ...
    if isinstance(initial_dataframe.index, pd.MultiIndex):
        ...
        cases_cutoff_multiindex = pd.MultiIndex.from_product(
            [range(first_level_index.max() + 1, first_level_index.max() + 1 + new_rows),
             second_level_index])
        ...
        new_df = pd.DataFrame(columns=initial_dataframe.columns,
...
class TimeoutError(Exception):
    def __init__(self, max_total_time_seconds, current_total_time):
        ...
        message = "Total training and evaluation time is {:.2f} {}, exceeding the maximum threshold of {:.2f} {}".format(
            current_total_time_seconds_value, current_total_time_seconds_unit, max_total_time_seconds_value,
...
class SearchBayesianSkopt(SearchAbstractClass):
    ...
    def _set_skopt_params(self,
        ...
        self.random_state = int(os.getpid() + time.time()) % np.iinfo(int).max
    ...
    def _resume_from_saved(self):
        ...
        try:
            ...
            if n_cases_in_loaded_data < self.n_cases:
                ...
                for dataframe_name in ["hyperparameters_df", "time_df", "result_on_validation_df", "result_on_test_df"]:
                    self.metadata_dict[dataframe_name] = _extend_dataframe(self.metadata_dict[dataframe_name],
                                                                           new_cases)
        ...
        for hyperparameter_index, hyperparameter_name in enumerate(self.hyperparams_names):
            if isinstance(self.hyperparams_values[hyperparameter_index], Categorical) and len(
                    self.hyperparams_values[hyperparameter_index].categories) == 1:
        ...
        for index in range(self.model_counter):
            ...
            is_validation_valid = result_on_validation_df is not None and \
                                  result_on_validation_df[self.metric_to_optimize].notna()[index].any()
    ...
    def _was_already_evaluated_check(self, current_fit_hyperparameters_dict):
        ...
        hyperparameters_df, current_fit_hyperpar_series = hyperparameters_df.align(current_fit_hyperpar_series, axis=1,
                                                                                   copy=False)
    ...
    def search(self, recommender_input_args,
        ...
        if self.max_total_time is not None:
            ...
            self._print(
                "{}: The search has a maximum allotted time of {:.2f} {}".format(self.ALGORITHM_NAME, total_time_value,
                                                                                 total_time_unit))
        ...
        for name, hyperparam in hyperparameter_search_space.items():
            ...
            if any(isinstance(hyperparam, sko_type) for sko_type in skopt_types):
                ...
                raise ValueError("{}: Unexpected hyperparameter type: {} - {}".format(self.ALGORITHM_NAME, str(name),
                                                                                      str(hyperparam)))
    ...
    def _objective_function_list_input(self, current_fit_hyperparameters_list_of_values):
        ...
        estimated_last_time = self.metadata_dict["time_df"].loc[self.metadata_dict['hyperparameters_best_index']][
            ["train", "validation"]].sum() if \
