...
class _Encoder(torch.nn.Module):
    ...
    def __init__(self, options, dropout_p=0.5, q_dims=None):
        ...
        for i in range(len(q_dims) - 1):
            ...
            self._network.add_module("layer_{}".format(i),
...
class _MultiVAEModel(torch.nn.Module):
    ...
    def get_l2_reg(self):
        ...
        for k, m in self.state_dict().items():
            if k.endswith(
                    '.weight'):  # The original MultVAE implementation only regularizes on the weights and not the biases
...
class MultVAERecommender_PyTorch(BaseRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self,
        ...
        assert p_dims[
                   -1] == self.n_items, "p_dims inconsistent with data, first dimension expected to be the number of items"
        ...
        self._model.load_state_dict(
            {key: torch.from_numpy(value).to(self.device) for key, value in self._model_state_best.items()},
            strict=True)
    ...
    def load_model(self, folder_path, file_name=None):
        ...
        self._model.load_state_dict(
            {key: torch.from_numpy(value).to(self.device) for key, value in data_dict["_model_state"].items()},
            strict=True)
...
class MultVAERecommender_PyTorch_OptimizerMask(MultVAERecommender_PyTorch):
    ...
    def fit(self, epochs=100,
        ...
        p_dims = generate_autoencoder_architecture(encoding_size, self.n_items, next_layer_size_multiplier,
                                                   max_parameters, max_n_hidden_layers)

        self._print("Architecture: {}".format(p_dims))

        super(MultVAERecommender_PyTorch_OptimizerMask, self).fit(epochs=epochs, batch_size=batch_size, dropout=dropout,
