...
class RP3betaRecommender(BaseItemSimilarityMatrixRecommender):
    ...
    def fit(self, alpha=1., beta=0.6, min_rating=0, topK=100, implicit=True, normalize_similarity=True, tail=False, tail_weight=0.05):
        ...
        similarity_builder = Incremental_Similarity_Builder(Pui.shape[1], initial_data_block=Pui.shape[1] * self.topK,
                                                            dtype=np.float32)
        ...
        for current_block_start_row in range(0, Pui.shape[1], block_dim):
            ...
            for row_in_block in range(block_dim):
                ...
                similarity_builder.add_data_lists(
                    row_list_to_add=np.ones(len(row_data), dtype=np.int32) * (current_block_start_row + row_in_block),
