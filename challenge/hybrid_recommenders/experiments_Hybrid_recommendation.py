...
def __main__():
    ...
    EASE_Wsparse = EASE_recommender.fit(topK=10, l2_norm=101, normalize_matrix=False)

    recommender_object = ItemKNNSimilarityHybridRecommender(URM_train, RP3beta_Wsparse, EASE_Wsparse)
    hybrid_Wsparse = recommender_object.fit(topK=56, alpha=0.8126786752572159)

    # recommender_object = ItemKNNSimilarityHybridRecommender(URM_train, P3alpha_Wsparse, hybrid_Wsparse)
