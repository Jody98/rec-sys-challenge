...
def train_recommenders(URM_train):
    ...
    hybrid_KNN = ItemKNNSimilarityTripleHybridRecommender(URM_train, p3alpha_Wsparse, item_Wsparse, RP3_Wsparse)
    hybrid_KNN.fit(topK=100, alpha=0.4976629488640914, beta=0.13017801200221196)
    ...
    all = HybridLinear(URM_train, recommenders)
    all.fit(eta=14.180249222221073, gamma=-0.38442274063330273,
...
def __main__():
    ...
    space = {
        'n_estimators': hp.choice('n_estimators', [5, 10, 20, 50, 100, 150]),
    ...
    n_estimators_choices = [5, 10, 20, 50, 100, 150]
    ...
    initial_recommender = recommenders["MultVAE"]
    evaluate_on_validation_set(recommenders, val_data, cutoff_list)
    recommenders.pop("MultVAE")
    ...
    def evaluate(params):
        model = xgb.XGBRanker(objective='rank:pairwise', **params, enable_categorical=True, booster='gbtree',
                              early_stopping_rounds=10, eval_metric='map@10')
    ...
    trials = Trials()
    best_indices = fmin(fn=evaluate, space=space, algo=tpe.suggest, max_evals=4000, trials=trials)
    ...
    recommenders_full = train_recommenders_full(URM_all)

    initial_recommender = recommenders_full["MultVAE"]
    evaluate_on_validation_set(recommenders_full, val_data, cutoff_list)
    recommenders_full.pop("MultVAE")
    ...
    # best_params = {'grow_policy': 'depthwise', 'learning_rate': 0.004214363159619697, 'max_depth': 0, 'max_leaves': 0,
    #                'n_estimators': 5, 'reg_alpha': 6.575950585977699, 'reg_lambda': 2.930342275422631}
